...
var validSeederName = regexp.MustCompile(`^[a-z][a-z0-9_]*$`)
...
var makeSeederCmd = &cobra.Command{
	...
	Run: func(cmd *cobra.Command, args []string) {
		seederName := args[0]
		if !validSeederName.MatchString(seederName) {
			log.Fatalf("Invalid seeder name: %s. Use only lowercase letters, numbers, and underscores.", seederName)
		}
