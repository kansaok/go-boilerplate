...
var makeMigrationCmd = &cobra.Command{
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		if !validMigrationName.MatchString(migrationName) {
			log.Fatalf("Invalid migration name: %s. Use only lowercase letters, numbers, and underscores.", migrationName)
		}
...
var migrateFreshCmd = &cobra.Command{
    ...
    Run: func(cmd *cobra.Command, args []string) {
		...
		if sqlDB, ok := dbConn.(*gorm.DB); ok {
			...
			defer sqlConn.Close()
			if err := db.FreshMigrate(sqlDB); err != nil {
				log.Fatalf("Fresh migrate failed: %v", err)
			}
		} else {
			log.Fatalf("Expected *gorm.DB but got %T", dbConn)
		}
